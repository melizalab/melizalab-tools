...
def param_sort(params):
    """
    Resorts parameters in a list according to motif, feature, and lag
    Useful for debugging a design matrix

    Returns an index array
    """
    tbl = nx.rec.recarray((params.size,), formats=['S4','i4','i4'], names=('mot','feat','lag'))
    for i,param in enumerate(params):
        mot,feat,lag = param.split('_')
        tbl[i] = mot, int(feat), int(lag)

    return tbl.argsort(order=('mot','feat','lag'))
...
def make_additive_model(rtls, mdb, ftablefun=readftable, fparams=None, trialave=True, **kwargs):
    ...
    tmax    - specify the last time point to include in the output matrix.
              default is to figure this out from the feature table 
    ...
    for stim, ftable in ftables.items():
        ...
        if def_tmax != None:
            ...
            if kwargs.get('ragged',_ragged):
                tmax = (ftable['fstart'] + ftable['flen']).max() + (nlags+1)*binsize
            else:
                tmax = ftable['fstart'].max() + (nlags+1)*binsize
...
def fit_additive_model(X,Y, **kwargs):
    ...
    #lu = sparse.linalg.factorized(CSS)
    #B = lu(CSR)

    return B,CSS


def model_err(B,X,XtX,Y):
    """
    Return OLS standard errors for Y = X*b

    B - model coefficients
    X - design matrix
    XtX - variance-covariance matrix
    Y - responses

    Returns:
    y_hat - predicted response
    b_err - vector same length as B with standard errors
    """
    Yhat = B * X.T
    S = nx.power(Yhat-Y,2).sum()
    SE = nx.sqrt(S/(X.shape[0]-X.shape[1]) * nx.diag(linalg.inv(XtX.todense())))
    return Yhat, SE
...
def makefrf(AA, ind=None, fill=0, descending=False):
    """
    Generate a FRF - a two dimensional array in which each row is the impulse response
    to a feature.  The rows are ranked in order of the mean response

    AA - dictionary with responses (i.e. generated by reshape_parameters)
    ind - optional, sort rows by this index instead of mean response
    fill - default 0; value of the array where the FRF is undefined
    descending - if indices are calculated, sort in descending order (default ascending)
    """

    maxlen = max([x.size for x in AA.values()])
