...
def train_classifier(args):
    import yaml
    import ewave
    from quicksong import streaming, core, features, model
    ...
    for dset in cfg["datasets"]:
        ...
        conv = streaming.OverlapSaveConvolver(core.make_hanning_kernels(cfg["smoothing"]["kernel_widths"]))
        extractor = core.FeatureExtractor(speccr, feats, conv)
        ...
        with open(path + ".lbl", "rt") as fp:
            ...
            X, y = core.make_training_dataset(feats, labels, cfg["spectrogram"]["shift_ms"])
        ...
        with open(args.model, "wb") as fp:
            pickle.dump({"config": cfg, "classifier": classifier}, fp)
